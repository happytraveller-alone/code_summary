...
Abstract:
    ...
    This module implements generic IPv6 subroutines.
...
Routine Description:
    ...
    Called once every timer tick from Ipv6pCompartmentSetTimeout.
...
--*/ 
{
    ...
    for (Link = InterfaceSet->Set.Flink;
        ...
        //
        // Handle anycast advertisement timeouts. 
        //
        if (!TtIsTableEmpty(Interface->AnycastAdvertisementTimerTable)) {
            Ipv6pHandleAnycastAdvertisementTimeout(Interface);
        }
        ...
        if ((Interface->RouterDiscoveryTimer != 0) ||
            ...
            ((!IsListEmpty(&Interface->PotentialRouterList)) &&
             (!Interface->Advertise))) {
            Ipv6pRouterDiscoveryTimeout(Interface, ForceRouterAdvertisement);
        }
