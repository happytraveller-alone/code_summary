...
Abstract:
    ...
    This module implements the functions of the IPv6 Control Sender module.
...
--*/
{
    ...
    case NsiSetCreateOrSetWithReference:
        ...
        Status = Ipv6SetEchoRequestCreate(Args, Protocol);
        break;
...
--*/    
{
    PIPV6_ECHO_REQUEST_CONTEXT EchoRequest = NULL;
    PIPV6_ECHO_REQUEST_RW EchoRequestRw;
    KIRQL OldIrql;
    NTSTATUS Status = STATUS_SUCCESS;
    ...
    USHORT HopByHopOptionsLength = 0;

    SIZE_T BytesCopied;
    PIPV6_ROUTING_HEADER RoutingHeader = NULL;
    ...
    //
    EchoRequestRw = (PIPV6_ECHO_REQUEST_RW)
    ...
    if (EchoRequestRw->Reverse) {
        RoutingHeaderLength = sizeof(IPV6_ROUTING_HEADER) + sizeof(IN6_ADDR);
    ...
    Status = 
        RtlULongAdd(
            EchoRequestPacketSize,
            ALIGN_UP(HopByHopOptionsLength, ULONGLONG),
            &EchoRequestPacketSize);
    if (!NT_SUCCESS(Status)) {
        goto EchoCleanup;
    }                        
    ...
    if (EchoSequence < IP_ECHO_REQUEST_MIN_SEQUENCE) {
        ...
        EchoSequence += Protocol->EchoRequestSequence;
    }
    ...
    if (!IN6_IS_ADDR_UNSPECIFIED(&EchoRequestRw->SourceAddress)) {
    ...
    if (!NT_SUCCESS(Status)) {
        NetioTrace(NETIO_TRACE_SEND, TRACE_LEVEL_VERBOSE, 
                   "IPNG: V6Echo request - can't find path to use.\n");
        goto EchoCleanup;
    }
    ...
    // like this. 
    // ----------------------------------------------------
    // | Hop by Hop Options/Routing header| TTL                            | 
    // ----------------------------------------------------
    //
    ...
    //
    // If we need to create a routing header, do that here.
    //
    if (RoutingHeaderLength > 0) {
        ...
        Object->cmsg_type = IP_RTHDR;
        AncilliaryDataOffset = (ULONG)Object->cmsg_len;
        
        RoutingHeader = 
            (PIPV6_ROUTING_HEADER)WSA_CMSG_DATA(Object);
        RoutingHeader->Length = 
            IPV6_EXTENSION_HEADER_BLOCKS(RoutingHeaderLength);
        RoutingHeader->RoutingType = 0;
        RoutingHeader->SegmentsLeft = 1;
        
        //
        // The destination address specified is actually an intermediate
        // destination.  The final destination is ourselves.
        //
        RtlCopyMemory((RoutingHeader + 1), 
                      DestinationAddress,
                      sizeof(IN6_ADDR));

        DestinationAddress = NL_ADDRESS(Path->SourceAddress);
        DestinationScopeId = NL_ADDRESS_SCOPE_ID(Path->SourceAddress);
        IN6_UNCANONICALIZE_SCOPE_ID((PIN6_ADDR) DestinationAddress, 
                                    &DestinationScopeId);
    }
    ...
    //
    // Start the timer to expire this send request.
    //
    ...
    ControlMessageRequest.Type = ICMP6_ECHO_REQUEST;
    ControlMessageRequest.DestProtocol = IPPROTO_ICMPV6;
